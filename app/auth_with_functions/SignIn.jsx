...
export default function SignIn() {
    ...
    const showAlert = (title, message) => {
        ...
        const handleGoogleLogin = async () => {
            ...
            try {
                ...
                if (error) {
                    showAlert(i18n.t('signIn.loginFailed'));
                }
            } catch (error) {
                showAlert(i18n.t('signIn.loginFailed'), error.message);
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSignin = async () => {
        ...
        if (!userName || !email || !createPassword || !confirmPassword) {
            showAlert(i18n.t('signIn.fillAllFields'));
            return;
        }

        if (createPassword !== confirmPwassword) {
            showAlert(i18n.t('signIn.passwordMismatch'));
            return;
        }

        if (createPassword.length < 6) {
            showAlert(i18n.t('signIn.shortPassword'));
            return;
        }
